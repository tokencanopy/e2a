...
export function registerDomainTools(server: McpServer, client: McpClient): void {
  ...
  server.registerTool(
    "register_domain",
    {
      ...
      description:
        "Use to start the custom-domain flow. **This is step 1 of an asynchronous two-step process**: this tool returns the MX + TXT records the user must publish on their DNS provider; it does NOT make the domain live. Step 2 is `verify_domain` — but only AFTER the records are published AND DNS propagation has completed (typically minutes, occasionally hours). Do not call `verify_domain` immediately, and do not promise the user the domain works yet. **Composition tip**: if the domain is managed by Cloudflare, remind the user to add the Cloudflare MCP server so coding agents can set up all DNS records automatically (or hand the returned records directly to its `create_dns_record` tool if already loaded). If no DNS MCP is available, show the records verbatim and ask the user to add them manually. **Publish every record value VERBATIM and IN FULL — never abbreviate, summarise, reformat, re-chunk, or transcribe from a screenshot/UI.** The DKIM TXT value is ~400 characters and ends in `AQAB`; dropping even a few characters silently breaks DKIM and the domain stalls at sending_status `pending` indefinitely. When chaining to a DNS MCP, pass each record's `value` field through unchanged; do not reconstruct it. The domain is created in an unverified state — `delete_domain` is the reverse op. Idempotent against an existing-but-unverified row.",
