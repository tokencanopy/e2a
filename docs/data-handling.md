...
| External sending access audit (operator grant/revoke: account id, old/new state, actor, reason; no addresses) | Postgres `external_sending_access_events` | Append-only; no account foreign key, so it outlives account deletion until its `expires_at` (the `sending_control_audit_retention_days` policy value, 90 days by default). |
