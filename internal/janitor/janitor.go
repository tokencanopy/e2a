...
func (j *Janitor) Sweep(ctx context.Context) error {
	...
	if j.engagements != nil {
		if drift, err := j.engagements.ReconcileEngagementCounts(ctx, "", engagementReconcileBatch); err != nil {
			...
			// Deliberately NOT reported through JanitorRowsDeleted: nothing was
			// deleted. Bending a pruning counter to describe a consistency
			// repair is how a metric stops meaning what its name says. The log
			// line above is the signal until this has a counter of its own.
		}
	}
