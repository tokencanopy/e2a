...
func (s *Store) Upsert(ctx context.Context, userID string, l Limits) error {
	_, err := s.pool.Exec(ctx,
		`INSERT INTO account_limits
		    (user_id, plan_code, max_agents, max_domains, max_messages_month, max_messages_day, max_storage_bytes, upgrade_url, updated_at)
		 ...
		 ON CONFLICT (user_id) DO UPDATE SET
		    ...
		    updated_at         = now()`,
		userID, l.PlanCode, l.MaxAgents, l.MaxDomains, l.MaxMessagesMonth, l.MaxMessagesDay, l.MaxStorageBytes, l.UpgradeURL,
	)
	return err
}
