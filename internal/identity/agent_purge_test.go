...
func (f purgeFixture) seedMessages(t *testing.T, n int, prefix string) {
	...
	if _, err := f.pool.Exec(context.Background(),
		`INSERT INTO messages (id, agent_id, direction, sender, recipient, subject)
		 SELECT $2 || i, $1, 'inbound', 'a@gmail.com', $1, 'seed'
...
func (f purgeFixture) seedCascadeChildren(t *testing.T) {
	...
	if _, err := f.pool.Exec(ctx,
		`INSERT INTO message_recipients (id, message_id, address)
		 SELECT 'mr_' || id, id, 'a@gmail.com' FROM messages WHERE agent_id = $1`,
		f.agentID); err != nil {
...
// TestDrainAgentChunksAbortsWhenRestoredMidPurge is the data-loss guard. If
// deleted_at is cleared while the drain is running, every remaining chunk must
// be refused — the inbox is live mail again.
func TestDrainAgentChunksAbortsWhenRestoredMidPurge(t *testing.T) {
	const seeded = 20000
	...
	f.onFirstChunkCommitted(t, seeded, func() {
		// RestoreAgent's own path underneath the running drain.
		_, _ = f.pool.Exec(context.Background(),
			`UPDATE agent_identities SET deleted_at = NULL WHERE id = $1`, f.agentID)
	})

	deleted, err := f.store.DrainAgentChunksForTest(context.Background(), f.agentID, f.userID)
	if !errors.Is(err, identity.ErrNotInTrash) {
		t.Fatalf("drain of a restored agent = %v (deleted %d), want ErrNotInTrash", err, deleted)
	}
	if n := f.countTable(t, "messages"); n == 0 {
		t.Error("drain emptied the whole inbox despite the restore — this is the data-loss case the guard exists to prevent")
	}
	if n := f.countTable(t, "agent_identities"); n != 1 {
		t.Error("drain deleted the restored agent row")
	}
}
...
func TestDeleteAgentAboveThresholdCountsRowsActuallyRemoved(t *testing.T) {
	...
	f.onFirstChunkCommitted(t, seeded, func() {
		tag, err := f.pool.Exec(context.Background(),
			`INSERT INTO messages (id, agent_id, direction, sender, recipient, subject)
			 SELECT 'mlate_' || i, $1, 'inbound', 'a@gmail.com', $1, 'late'
...
// TestDeleteAgentAboveThresholdPreservesTrashTimestamp: permanently deleting an
// agent that is ALREADY in the trash must not rewrite deleted_at while the
// chunked drain runs. Callers can see that timestamp and the janitor's
// retention sweep keys on it.
func TestDeleteAgentAboveThresholdPreservesTrashTimestamp(t *testing.T) {
	...
	case during := <-seen:
		...
		if !during.Equal(before) {
			t.Errorf("deleted_at moved %v → %v — a permanent delete must not reset the trash clock", before, *during)
		}
	case <-time.After(10 * time.Second):
