...
export class E2AAuthError extends E2AError {
} // 401
export class E2APermissionError extends E2AError {
} // 403
export class E2ANotFoundError extends E2AError {
} // 404 / 410
export class E2AConflictError extends E2AError {
} // 409
export class E2AValidationError extends E2AError {
} // 422 — input validation
export class E2AIdempotencyError extends E2AError {
} // idempotency_in_flight / _key_reuse
export class E2ALimitExceededError extends E2AError {
} // 402 — QUOTA (stock/flow) cap; NOT retryable
export class E2ARateLimitError extends E2AError {
} // 429 — request-RATE / throughput limit; retryable
export class E2AServerError extends E2AError {
} // 5xx / 408
export class E2AConnectionError extends E2AError {
} // no response (network)
export class E2AWebhookSignatureError extends E2AError {
} // local webhook verify failure
/**
 ...
 */
export class E2AConnectionReplacedError extends E2AError {
...
const mkAuth = (f) => new E2AAuthError(f);
const mkPermission = (f) => new E2APermissionError(f);
const mkNotFound = (f) => new E2ANotFoundError(f);
const mkConflict = (f) => new E2AConflictError(f);
const mkValidation = (f) => new E2AValidationError(f);
const mkIdempotency = (f) => new E2AIdempotencyError(f);
const mkLimitExceeded = (f) => new E2ALimitExceededError(f);
const mkRateLimit = (f) => new E2ARateLimitError(f);
const mkServer = (f) => new E2AServerError(f);
...
function resolve(status, code) {
    ...
    if (code) {
        ...
        if (code.endsWith("_taken") || code.endsWith("_exists"))
    ...
    if (isRetryableStatus(status))
        return { make: mkServer, retryable: true };
...
export function parseErrorEnvelope(raw) {
    let body = raw;
...
export function fromApiException(e) {
    ...
    if (env && env.error) {
        ...
        const bodyReqId = env.error.request_id;
        if (!requestId && typeof bodyReqId === "string")
            requestId = bodyReqId;
    }
