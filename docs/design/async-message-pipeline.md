...
- **`ON DELETE CASCADE` caveat is gone** for the queue (there is no FK'd queue row). Every irreversible message-owning deletion path (`PurgeMessage`, `DeleteAgent`, `PurgeDeletedAgents`, `DeleteExpiredMessages`, and user-data deletion) must still **cancel each linked River send job in the same transaction**. If cancellation fails, the deletion rolls back; if River already pruned the job, treat that as success. Reversible trash deliberately retains the job so restore can re-arm a scheduled send.
