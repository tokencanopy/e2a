...
import { E2AError, E2AAuthError, E2AConnectionReplacedError, E2APermissionError, E2ANotFoundError, } from "./errors.js";
...
function fatalErrorForStatus(status) {
    const message = `WebSocket handshake rejected: HTTP ${status}`;
    if (status === 401)
        return new E2AAuthError({ code: "unauthorized", message, status, retryable: false });
    if (status === 403)
        return new E2APermissionError({ code: "forbidden", message, status, retryable: false });
    ...
    if (status === 404)
        return new E2ANotFoundError({ code: "not_found", message, status, retryable: false });
...
function fatalErrorForClose(code, reason) {
    ...
    dial() {
        ...
        ws.on("unexpected-response", (_req, res) => {
            ...
            if (status >= 400 && status < 500)
                fatal = fatalErrorForStatus(status);
        });
        ws.on("open", () => {
 ...
 */
export class WSStream extends EventEmitter {
    listener;
    ...
    deliver(notif) {
        ...
        if (this.buffer.length > WS_MAX_BUFFERED_EVENTS) {
            ...
            if (!this.bufferOverflowed) {
                ...
                console.warn(`[e2a] WSStream dropped events: more than ${WS_MAX_BUFFERED_EVENTS} un-consumed notifications buffered. The consumer is not keeping up — do less work inside the for-await loop or fetch out of band.`);
            }
        }
    }
