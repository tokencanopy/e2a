...
| `e2a_oidc_callback_total` | counter | `outcome`, `trust`, `status_class` | Browser callback outcomes. `outcome` is `success`, `discovery_unavailable`, `state_invalid`, `provider_rejected`, `provider_failed`, `response_invalid`, `token_exchange_failed`, `id_token_invalid`, `claim_invalid`, `unknown_user`, `session_failed`, or `post_login_failed`. `provider_rejected` is the expected `access_denied` user refusal/cancellation path; every other provider-returned OAuth error is the bounded actionable `provider_failed` category. `trust=public` covers requests rejected before browser transaction state is validated; `trust=trusted` means the server-authenticated state and transaction cookies matched first. `status_class` is e2a's response class. Alert on sustained actionable `trusted` failures (including `provider_failed`, token exchange, invalid claim, unknown user, or session failure), not scanner-shaped `public` traffic or user-denied `provider_rejected` callbacks. Raw provider codes and descriptions are never labels or log fields. |
