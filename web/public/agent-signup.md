...
Public self-signup always creates the provisional inbox on the deployment's
shared agent domain. An unauthenticated request never claims an address on an
account-owned custom domain; custom-domain inbox creation remains an
authenticated account operation.
