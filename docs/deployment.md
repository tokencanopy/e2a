...
| `GEMINI_BASE_URL` | no (default AI Studio, `https://generativelanguage.googleapis.com/v1beta`) | Overrides the Gemini REST base URL. The detector appends `/models/{model}:generateContent`, so a project-scoped Vertex AI base of the shape `https://{region}-aiplatform.googleapis.com/v1/projects/{project}/locations/{region}/publishers/google` works unchanged. **Compliance note:** the detector sends the message subject, sender, and up to 4,000 characters of body text to this endpoint. The AI Studio default is not covered by a Google Cloud BAA/DPA; deployments with confidentiality obligations (e.g. HIPAA) must route through Vertex AI under their Google Cloud agreement (with `GEMINI_AUTH=adc`) or leave scanning disabled. |
| `GEMINI_AUTH` | no (default `api_key`) | How the Gemini call authenticates: `api_key` sends `GEMINI_API_KEY`/`GOOGLE_API_KEY` in the `x-goog-api-key` header (AI Studio); `adc` sends an OAuth bearer token minted from Application Default Credentials (workload identity, service-account file, or `gcloud auth application-default`) — required for project-scoped Vertex AI endpoints, where no API key is used. Any other value fails detector construction loudly (logged at startup) rather than silently falling back. |
