...
async function sentinelTarget() {
  const root = await mkdtemp(join(tmpdir(), "e2a-umami-tracker-"));
  roots.push(root);
  const target = pathToFileURL(join(root, "umami.js"));
...
test("update pins the fixed request and preserves an existing target when validation rejects", async () => {
  const failures = [
    ...
    },
    {
      ...
      error: /has 1 bytes; expected 4655/,
    },
    {
