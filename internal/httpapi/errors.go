...
func (ErrorBody) TransformSchema(r huma.Registry, s *huma.Schema) *huma.Schema {
	detailTypes := map[string]reflect.Type{
		...
		"ExternalSendingNotEnabledDetails": reflect.TypeOf(ExternalSendingNotEnabledDetails{}),
	}
...
// ExternalSendingNotEnabledDetails is the typed `error.details` payload of a
// 403 external_sending_not_enabled response. It names what the account CAN
// send to right now and where the authenticated recovery flow lives. It is a
// permission answer, not a quota (402) or a transient condition: retrying the
// same request is refused the same way until the account's access changes.
type ExternalSendingNotEnabledDetails struct {
	AllowedRecipients []string `json:"allowed_recipients" doc:"Destinations this account may send to through its shared sending identity right now. Open set: treat values as strings and tolerate unknown ones. Known values: verified_owner_email (the account's verified sign-in email), same_account_agents (live agent inboxes in this account)."`
	RecoveryURL       string   `json:"recovery_url,omitempty" doc:"Authenticated dashboard page that explains the restriction and offers the recovery routes (verify a sending domain, request approval, or — on the hosted service — a paid base plan)."`
}
