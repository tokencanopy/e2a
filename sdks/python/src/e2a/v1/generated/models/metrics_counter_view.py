...
class MetricsCounterView(BaseModel):
    """
    MetricsCounterView
    """ # noqa: E501
    messages: StrictInt = Field(description="Distinct messages carrying at least one observation of this code. This is the grain funnel arithmetic uses.")
    observations: StrictInt = Field(description="Ledger rows. Retryable codes are recorded once per attempt, so this exceeds messages whenever the pipeline retried.")
    outcome: StrictStr = Field(description="What e2a observed at that stage.")
    reason_code: StrictStr = Field(description="Canonical lifecycle reason code.")
    retryable: StrictBool = Field(description="Whether this observation describes a condition the pipeline retries.")
    stage: StrictStr = Field(description="Lifecycle stage the observation was made at.")
    additional_properties: Dict[str, Any] = {}
    __properties: ClassVar[List[str]] = ["messages", "observations", "outcome", "reason_code", "retryable", "stage"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of MetricsCounterView from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of MetricsCounterView from a dict"""
        if obj is None:
            return None
