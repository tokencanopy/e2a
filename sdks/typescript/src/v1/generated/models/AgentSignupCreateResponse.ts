/**
 * e2a API
 * e2a — authenticated email gateway for AI agents. v1 contract.  ## Stability policy  The v1 surface is stable and evolves **additively only**: new endpoints, new optional request fields, new response fields, and new values in open string sets (event types, statuses) may appear at any time without a version bump. Clients MUST tolerate unknown response fields and unknown values in open string sets. This is machine-readable in the schemas: response schemas declare `additionalProperties: true`; request schemas stay strict (`additionalProperties: false` — an unknown request field is rejected with 422).  Operations and schemas marked `x-stability-level: beta` are exempt from this freeze and may change or be removed without a major version. A field marked `x-experimental-values` is itself stable, but the listed values (and their event payloads) are experimental. Everything not marked beta, or enumerated as experimental, is stable.  Removing or changing stable surface only happens on a new major version path (/v2); deprecations are announced ahead of time via `deprecated: true` in this document and keep working within v1.
 *
 * OpenAPI spec version: 1.0.0
 *
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { AgentSignupRestrictionsView } from '../models/AgentSignupRestrictionsView.js';
import { HttpFile } from '../http/http.js';

export class AgentSignupCreateResponse {
    /**
    * One-time agent-scoped API key. Store it now; a re-signup rotates it.
    */
    'apiKey': string;
    'consoleUrl'?: string;
    'createdAt': Date;
    'displayName': string;
    'harness'?: string;
    'humanEmail': string;
    'id': string;
    /**
    * Provisioned inbox on the deployment shared agent domain.
    */
    'inbox': string;
    'noteToHuman'?: string;
    'restrictions': AgentSignupRestrictionsView;
    'reviewOutbound': boolean;
    /**
    * Open lifecycle value. Known values: pending, verified, rejected.
    */
    'status': string;
    'verificationExpiresAt': Date;

    static readonly discriminator: string | undefined = undefined;

    static readonly mapping: {[index: string]: string} | undefined = undefined;

    static readonly attributeTypeMap: Array<{name: string, baseName: string, type: string, format: string}> = [
        {
            "name": "apiKey",
            "baseName": "api_key",
            "type": "string",
            "format": ""
        },
        {
            "name": "consoleUrl",
            "baseName": "console_url",
            "type": "string",
            "format": ""
        },
        {
            "name": "createdAt",
            "baseName": "created_at",
            "type": "Date",
            "format": "date-time"
        },
        {
            "name": "displayName",
            "baseName": "display_name",
            "type": "string",
            "format": ""
        },
        {
            "name": "harness",
            "baseName": "harness",
            "type": "string",
            "format": ""
        },
        {
            "name": "humanEmail",
            "baseName": "human_email",
            "type": "string",
            "format": ""
        },
        {
            "name": "id",
            "baseName": "id",
            "type": "string",
            "format": ""
        },
        {
            "name": "inbox",
            "baseName": "inbox",
            "type": "string",
            "format": ""
        },
        {
            "name": "noteToHuman",
            "baseName": "note_to_human",
            "type": "string",
            "format": ""
        },
        {
            "name": "restrictions",
            "baseName": "restrictions",
            "type": "AgentSignupRestrictionsView",
            "format": ""
        },
        {
            "name": "reviewOutbound",
            "baseName": "review_outbound",
            "type": "boolean",
            "format": ""
        },
        {
            "name": "status",
            "baseName": "status",
            "type": "string",
            "format": ""
        },
        {
            "name": "verificationExpiresAt",
            "baseName": "verification_expires_at",
            "type": "Date",
            "format": "date-time"
        }    ];

    static getAttributeTypeMap() {
        return AgentSignupCreateResponse.attributeTypeMap;
    }

    public constructor() {
    }
}
