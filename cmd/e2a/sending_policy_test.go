...
func TestExternalSendingOperatorCommands(t *testing.T) {
	...
	if _, err := pool.Exec(ctx, `INSERT INTO account_limits (user_id, max_agents, max_domains, max_messages_month, max_storage_bytes, external_sending_entitled) VALUES ('usr_cmd_esa', 1, 1, 1, 1, true)`); err != nil {
		t.Fatal(err)
	}
