...
function validateOptions({ root, suiteName, targetEnv, actorEnv }) {
...
function expandTemplate(source, { suiteName, targetEnv, actorEnv }) {
  const replacements = {
...
function parseArguments(args) {
  const values = {};
  const optionNames = new Set(["--root", "--name", "--target-env", "--actor-env"]);
  if (args.length % 2 !== 0) throw new Error("Usage: scaffold.mjs --root <suite-root> --name <suite-name> --target-env <env> --actor-env <env>");
  for (let index = 0; index < args.length; index += 2) {
    const option = args[index];
    const value = args[index + 1];
    if (!optionNames.has(option) || !value || values[option] !== undefined) {
      throw new Error("Usage: scaffold.mjs --root <suite-root> --name <suite-name> --target-env <env> --actor-env <env>");
    }
    values[option] = value;
  }
  if (["--root", "--name", "--target-env", "--actor-env"].some((option) => values[option] === undefined)) {
    throw new Error("Usage: scaffold.mjs --root <suite-root> --name <suite-name> --target-env <env> --actor-env <env>");
  }
  return {
    root: values["--root"],
